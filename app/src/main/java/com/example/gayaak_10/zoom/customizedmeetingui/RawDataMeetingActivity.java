...
import com.example.gayaak_10.zoom.customizedmeetingui.rawdata.AudioRawDataUtil;
import com.example.gayaak_10.zoom.customizedmeetingui.rawdata.RawDataRender;
import com.example.gayaak_10.zoom.customizedmeetingui.rawdata.UserVideoAdapter;
import com.example.gayaak_10.zoom.customizedmeetingui.rawdata.VirtualVideoSource;
...
//                meetingShareHelper.startShareScreenSession(mScreenInfoData);
    ...
    public void updateAudioButton() {
        ...
        if (audioController.isAudioConnected()) {
            if (audioController.isMyAudioMuted()) {
                audioStatusImage.setImageResource(R.drawable.ic_mic_off);
            } else {
                audioStatusImage.setImageResource(R.drawable.ic_mic);
            }
        } else {
            audioStatusImage.setImageResource(R.drawable.ic_mic_off);
        }
    ...
    public void onUserVideoStatusChanged(long userId) {
        ...
        if (userId == myUserId) {
            if (!status.isSending()) {
                videoStatusImage.setImageResource(R.drawable.ic_videocam_off);
            } else {
                videoStatusImage.setImageResource(R.drawable.ic_videocam);
            }
        }
